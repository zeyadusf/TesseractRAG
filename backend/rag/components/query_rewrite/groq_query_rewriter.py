...
class GroqQueryRewriter(QueryRewriteBase):
    ...
    async def rewrite(self, query: str, history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        prompt = QueryReWritePrompt.CONTEXTUAL.format(query=query.strip(), history=history)
        try:
    ...
    async def expand(self, query: str, history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        prompt = QueryReWritePrompt.EXPAND.format(query=query.strip(), history=history)
        try:
            raw = await self._call_api(prompt, temperature=0.0)
    ...
    async def rewrite_and_expand(self, query: str, history: Optional[List[Dict[str, str]]] = None) -> dict[str, str]:
        rewritten, expanded = await asyncio.gather(
            self.rewrite(query, history),
            self.expand(query, history),
        )
        return {
            "original": query,
            "rewritten": rewritten,
